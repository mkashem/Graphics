...
void createSphere() {
		...
		for (double b = -STEP / 2.0; b <= STEP / 2.0; b++) {
			if (b == -STEP / 2.0 || b == STEP / 2.0) { 
				...
				if (b==-STEP/2.0) {
					textures[s_index].x = 0.0;
					textures[s_index].y = (1.0*b/STEP)+0.5;
				}
				if (b == STEP / 2.0) {
					textures[s_index].x = 1.0;
					textures[s_index].y = (1.0*b / STEP) + 0.5;
				}
...
///top**********************************
	...
	glBufferSubData(GL_ARRAY_BUFFER, offset, sizeof(textures), textures);
	...
	glVertexAttribPointer((GLuint)2, 2, GL_FLOAT, GL_FALSE, 0, (GLvoid *)sizeof(textures));
